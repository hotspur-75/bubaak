...
class FunctionCloningRewriter(Rewriter):
    ...
    def clone_call(self, state, cfa_edge, call_init_node):
        ...
        definition_clone          = _create_clone(function_definition.ast_node, clone_name)

        if clone_iter > 0:
            full_replacement = f"{definition_clone}\n\n// SKIP {clone_iter}\n{function_definition_text}"
        else:
            full_replacement = f"{definition_clone}\n\n{function_definition_text}"

        replacements.append((function_definition.ast_node, full_replacement))
        return replacements
